...
class RetrievalDataset_3DText(Dataset):
    def __init__(self, root, text_max_len, unimol_dict, max_atoms, tokenizer, args):
...
class RetrievalDataset_2DText(Dataset):
    def __init__(self, root, text_max_len, tokenizer, args):
...
class RetrievalDataset_D2D3(Dataset):
    def __init__(self, root, unimol_dict, max_atoms, args):
        ...
        self.graph_name_list_d2d3 = sorted(os.listdir(root+'graph/'), key = lambda x:int(x.split('.')[0].split('_')[1]))
