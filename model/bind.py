...
from model.bert import BertConfig, BertLMHeadModel
from transformers import BertTokenizer
from model.graph import GNN
...
class Blip2Base(BaseModel):
    ...
    @classmethod
    def init_bert_encoder(cls, model_name, num_query_token, graph_width, cross_attention_freq=2):
        ...
        bert = BertLMHeadModel.from_pretrained(
    ...
    def init_graph_encoder(
        ...
        ckpt = torch.load('./backbone/graph_pretrain.pth', map_location=torch.device('cpu'))
    ...
    def init_unimol_mol_encoder(cls, args):
        ...
        ckpt = torch.load('backbone/mol_pretrain.pt', map_location=torch.device('cpu'))['model']
    ...
    def init_unimol_pro_encoder(cls, args):
        ...
        dictionary.add_symbol("[MASK]", is_special=True)
        unimol_model = SimpleUniMolModel(args, dictionary)
        ckpt = torch.load('./backbone/protein_pretrain.pt', map_location=torch.device('cpu'))['model']
        missing_keys, unexpected_keys = unimol_model.load_state_dict(ckpt, strict=False)
        # if len(missing_keys) or len(unexpected_keys):
        #     print(missing_keys)
        #     print(unexpected_keys)
    ...
    def load_from_pretrained(self, url_or_filename):
        ...
        logging.info("loader checkpoint from %s" % url_or_filename)
