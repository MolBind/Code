...
def add_checkpoint_args(parser):
    ...
    group.add_argument('--restore-file', default='checkpoint_last.pt',
                       help='filename from which to loader checkpoint '
                            '(default: <save-dir>/checkpoint_last.pt')
    group.add_argument('--finetune-from-model', type=str,
                       help="finetune from a backbone model; note that meters and lr scheduler will be reset")
    group.add_argument('--loader-from-ema', action="store_true",
                       help="finetune from a backbone model; note that meters and lr scheduler will be reset")
    group.add_argument('--reset-dataloader', action='store_true',
                       help='if set, does not reload dataloader state from the checkpoint')
    group.add_argument('--reset-lr-scheduler', action='store_true',
                       help='if set, does not loader lr scheduler state from the checkpoint')
    group.add_argument('--reset-meters', action='store_true',
                       help='if set, does not loader meters from the checkpoint')
    group.add_argument('--reset-optimizer', action='store_true',
                       help='if set, does not loader optimizer state from the checkpoint')
